...
| CloudBase returns a Next.js `404` for `/api/health/origin` | HTTP service `/api` route is bound as generic `SCF` or path passthrough is off | Re-run `npm run cn:deploy` from the latest script; it recreates `/api` as `WEB_SCF` with `enablePathTransmission=true` and verifies the route |
