...
As a founder (player), I want to schedule a paid meetup with an expert (coach) by selecting a time slot and paying upfront—so we can share context and move forward together (learn by doing, grow by helping).
...
7. **Overview** step: total price, full amount due now, meetup details
8. On web, pay the full amount via **Stripe Checkout**
9. Stripe Checkout surfaces supported methods such as PayNow or card based on Stripe configuration and user context
10. On Telegram, pay the full amount via **TON Wallet** only
11. Booking confirms after the selected payment flow succeeds
12. There is no post-meetup remainder charge for new bookings
