...
The script retries transient Supabase pooler/control-plane failures such as **“Circuit breaker open: Failed to retrieve database credentials”**. After the retry budget, it allows the build to continue with a warning so a temporary database-side outage does not fail `npm install`. For deployments that must fail when migrations cannot run, set `PRISMA_MIGRATE_STRICT=1`. When a transient skip happens, run `npx prisma migrate deploy` manually once the database is reachable.
