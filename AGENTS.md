...
2. **AI providers**: `AI_PROVIDER` still supports multiple adapters in code (`dedalus`, `gemini`, `qwen`, `openai`, `zai`, `ollama`, `byteplus`, `volcengine`, `hunyuan`), but the public **Help-And-Grow/expert-network** deployment is standardized on **Alibaba Cloud Qwen / DashScope**. Default when unset: **`qwen`**. The containerized local stack defaults to **`ollama`**. **China WeChat app** (future) will use **`hunyuan`** (Tencent LLM) to ensure all AI processing stays within China mainland. Keep public docs, deployment config, and admin controls aligned with Alibaba/Qwen unless a task explicitly targets private experimentation. See `src/lib/ai/index.ts` and `src/lib/vendor-ai-stack-site.ts`.
...
6. **WeChat Mini Program**: Lives in `wechat/`, built with Taro. Two apps serve different markets:
   - **International app** (`TARO_APP_REGION=intl`, AppID `wx09d0eb079596060d`): Registered by Singapore company, positioned as a **free mentoring platform** helping youth learn AI in building products. Runs on Tencent CloudBase env `cn-wechat-d1gzncs8i34827c98`.
   - **China mainland app** (`TARO_APP_REGION=cn`): Future app registered by the Chinese company, connecting to China-local Tencent Cloud infrastructure with **Hunyuan** as AI provider. **Data residency principle**: all data for the China app is stored and processed **only within China mainland** (separate DB, separate infra). Build config: `wechat/build-config/cn.json`.
